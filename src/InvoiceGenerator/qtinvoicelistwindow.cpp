...
{
    emit dataBaseWindowEvent(UIE_GENERATE_INVOICE);
}

void QTInvoiceListWindow::addInvoiceEvent()
{
    emit dataBaseWindowEvent(UIE_ADD_NEW_INVOICE);
}
...
{
    emit dataBaseWindowEvent(UIE_BACK);
}
...
void QTInvoiceListWindow::invoiceClicked(const QModelIndex &index)
{
    ...
    {
        emit dataBaseWindowEvent(UIE_EDIT_INVOICE, index.row());
    }
    m_currentIndex = index;
}
